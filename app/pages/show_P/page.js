...
import { User, Mail, MapPin, Phone, Brain, Sparkles, Edit, Save, X, Camera, Calendar, Globe, LogIn } from 'lucide-react';
...
const UserProfilePage = () => {
  ...
  // --- MODIFIED: userId is now state, not hardcoded ---
  ...
  // --- MODIFIED: This effect now handles auth check and data fetching ---
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        if (decodedPayload.id) {
          ...
          // Token is invalid or doesn't contain an ID
          handleAuthError();
        }
      } catch (error) {
  ...
  // --- MODIFIED: handleSave now uses the userId from state ---
      ...
      Object.keys(editData).forEach(key => {
        ...
        if (key === 'profilePicture' && val instanceof File) {
          formData.append(key, val);
           ...
           // Ensure arrays of objects/strings are stringified
  ...
  // --- NEW: Render state for unauthenticated users ---
  if (!isAuthenticated) {
     return (
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
              <div className="relative inline-block mb-6">
                <div className="w-32 h-32 rounded-full bg-gradient-to-br from-blue-400 to-purple-400 flex items-center justify-center overflow-hidden border-4 border-white shadow-lg">
                  {(uploadedImage || userData.profilePicture) ? (
                    <img 
                      src={uploadedImage || `http://localhost:5000/${userData.profilePicture}`} 
                      alt="Profile" 
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex items-center justify-center gap-2 text-gray-600">
                  ...
                  <span>Born: {formatDate(userData.dateOfBirth)}</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
              ...
              {editMode ? (
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-lg text-gray-800 capitalize">{userData.gender || 'Not specified'}</p>
                  </div>
                  <div>
                    <span className="text-sm font-semibold text-gray-500">Date of Birth</span>
                    <p className="text-lg text-gray-800">{formatDate(userData.dateOfBirth)}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                <MapPin className="mr-3 text-green-500" /> Location
              </h3>
              ...
              {editMode ? (
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-lg text-gray-800">{userData.city}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-lg text-gray-800">{userData.country}</p>
                  </div>
                  <div>
                    <span className="text-sm font-semibold text-gray-500">ZIP Code</span>
                    <p className="text-lg text-gray-800">{userData.zipCode || 'Not provided'}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                <Brain className="mr-3 text-purple-500" /> Skills
              </h3>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  {editMode ? (
                      ...
                      {predefinedSkills.map(skill => (
                        <motion.button
                          ...
                          className={`p-3 rounded-xl text-sm font-medium transition-all ${
                            editData.skillsToTeach?.some(s => s.name === skill)
                    ...
                    <div className="flex flex-wrap gap-2">
                      {userData.skillsToTeach?.length > 0 ? userData.skillsToTeach.map((skill, index) => (
                        <span
                          key={index}
                      ...
                      )) : <p className="text-gray-500">No skills to teach listed</p>}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {editMode ? (
                    <div className="flex flex-wrap gap-3">
                      {predefinedSkills.map(skill => (
                        <motion.button
                          ...
                          className={`p-3 rounded-xl text-sm font-medium transition-all ${
                            editData.skillsToLearn?.includes(skill)
                    ...
                    <div className="flex flex-wrap gap-2">
                      {userData.skillsToLearn?.length > 0 ? userData.skillsToLearn.map((skill, index) => (
                        <span
                          key={index}
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                <Sparkles className="mr-3 text-orange-500" /> About & Contact
              </h3>
              ...
              {editMode ? (
                ...
                <div className="space-y-4">
                  <div>
                    <span className="text-sm font-semibold text-gray-500">Bio</span>
                    <p className="text-gray-800 mt-1">{userData.bio || 'No bio provided'}</p>
                  </div>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <span className="text-sm font-semibold text-gray-500">Mobile</span>
                      <p className="text-lg text-gray-800">{userData.mobile || 'Not provided'}</p>
                    </div>
                    <div>
                      <span className="text-sm font-semibold text-gray-500">Preferred Contact</span>
                      <p className="text-lg text-gray-800">{userData.preferredContact}</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {editMode && (
          ...
          >
            <motion.button
              ...
              onClick={handleSave}
              disabled={saving}
              className="px-8 py-4 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all flex items-center gap-2"
            >
              <Save size={20} />
