...
Object.keys(formData).forEach(key => {
  ...
  if (key === 'profilePicture' && val instanceof File) {
    ...
    formDataToSend.append(key, val); // Don't stringify strings
  }
});
  ...
  try {
    ...
    if (res.ok) {
      setSuccess(true);
      setTimeout(() => (window.location.href = '/pages/log_in'), 2000);
    } else {
      setErrorMessage(data.message || 'Server rejected the request');
    }
  } catch (err) {
    console.error(err);
    setErrorMessage('Network error – is the backend running on :5000?');
  } finally {
            ...
            <div className="space-y-8">
              <div>
                <h3 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                  <Brain className="mr-2 text-purple-500" /> Skills You Can Teach
                </h3>
                  ...
                  {predefinedSkills.map(skill => (
                    <motion.button
                      ...
                      className={`p-3 rounded-xl text-sm font-medium transition-all ${
                        formData.skillsToTeach.some(s => s.name === skill)
              ...
              <div>
                <h3 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                  <Sparkles className="mr-2 text-blue-500" /> Skills You Want to Learn
                </h3>
              ...
              <div key={s.id} className="flex items-center flex-1">
                ...
                {index < steps.length - 1 && (
                  <div className={`flex-1 h-1 mx-2 transition-all duration-300 ${
                    step > s.id ? 'bg-gradient-to-r from-blue-500 to-purple-500' : 'bg-gray-200'
                  }`} />
                )}
              </div>
            ))}
          ...
          <div className="flex justify-between mt-8">
            ...
            {step < 4 ? (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={nextStep}
                className="px-8 py-3 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all flex items-center gap-2"
              >
