...
// --- Self-contained SVG Icons (Replaced lucide-react to avoid errors) ---
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 py-8">
        ...
        {/* --- Our Story Section --- */}
        <div className="mb-16 bg-white/70 backdrop-blur-sm rounded-2xl p-8 shadow-lg border border-gray-100">
            <h2 className="text-3xl font-bold text-center mb-4 bg-gradient-to-r from-green-500 to-blue-500 bg-clip-text text-transparent">Our Story</h2>
        ...
        <div className="mb-16">
            <h2 className="text-3xl font-bold text-center mb-10 bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent">How It Works</h2>
        ...
        <div className="text-center bg-white/60 backdrop-blur-sm p-10 rounded-2xl shadow-xl">
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
        ...
        {isTermsOpen && (
            <div className="w-96 h-[500px] bg-white shadow-2xl rounded-xl flex flex-col overflow-hidden transition-all duration-300 ease-in-out">
                <div className="bg-gradient-to-r from-gray-700 to-gray-800 text-white p-3 flex justify-between items-center flex-shrink-0">
                    ...
                    <button onClick={() => setIsTermsOpen(false)} className="hover:opacity-75"><X size={18} /></button>
                </div>
        ...
        {isChatbotOpen && (
          <div className="w-96 h-[500px] bg-white shadow-2xl rounded-xl flex flex-col overflow-hidden transition-all duration-300 ease-in-out">
            <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-3 flex justify-between items-center flex-shrink-0">
              <span className="font-semibold">Skill Exchanger Bot</span>
              <button onClick={() => setIsChatbotOpen(false)} className="hover:opacity-75"><X size={18} /></button>
            </div>
            <div className="flex-1 p-4 overflow-y-auto space-y-4 text-sm">
              {messages.map((msg, i) => (
                <div key={i} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`p-3 rounded-2xl max-w-[85%] ${ msg.sender === 'bot' ? 'bg-gray-200 text-gray-800 rounded-bl-none' : 'bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-br-none'}`}>
                    {msg.text}
                  </div>
                  {msg.isLangSelector && (
        ...
        <div className="flex items-center gap-4">
            <button onClick={() => setIsTermsOpen(!isTermsOpen)} className="w-16 h-16 bg-gradient-to-r from-gray-600 to-gray-800 rounded-full shadow-lg flex items-center justify-center text-white hover:scale-110 transition-transform">
                <FileText size={24} />
            </button>
            <button onClick={() => setIsChatbotOpen(!isChatbotOpen)} className="w-16 h-16 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full shadow-lg flex items-center justify-center text-white hover:scale-110 transition-transform">
